...
public class MailboxProcessor implements Closeable {
    ...
    private boolean processMailsWhenDefaultActionUnavailable() throws Exception {
        ...
        while (isDefaultActionUnavailable() && isMailboxLoopRunning()) {
            ...
            System.out.println(
                    "process control message while waiting for input: " + mail.toString());
            mail.run();
    ...
    private boolean processMailsNonBlocking(boolean singleStep) throws Exception {
        ...
        while (isMailboxLoopRunning() && (maybeMail = mailbox.tryTakeFromBatch()).isPresent()) {
            ...
            Mail mail = maybeMail.get();
            System.out.println("process control message: " + mail.toString());
            mail.run();
            if (singleStep) {
                break;
            }
    ...
    public boolean isDefaultActionUnavailable() {
        return suspendedDefaultAction != null || isPaused;
    }
